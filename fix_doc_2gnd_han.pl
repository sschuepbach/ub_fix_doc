...
while ($line = <STDIN>) {                                           
   if ($line) {
      chomp $line; 	
      if (substr($line, 0, 5) =~ /6(00|10|11|30).7/) {      #Fall 1: Sacherschliessungfeld mit Indikator2=7
         ...
         if ( $line =~ /(\$\$1\(DE-588\).+?)(\$\$|$)/ ) {         #Pruefe ob $$1 (DE-588) * vorkommt
            ...
            $line =~ s/\$\$2.+?\$\$/\$\$/g;                       #Loesche allfaellig vorhandenes Unterfeld $$2
            $line =~ s/\$\$2.+?$//g;                              #Loesche allfaellig vorhandenes Unterfeld $$2
            $line =~ s/$/\$\$2resource/                           #Fuege Unterfeld $$2 ressource am Ende ein
         }                                           
      } elsif (substr($line, 0, 5) =~ /6(50|51).7/) {
         $line =~ s/\$\$.\$\$/\$\$/g;                             #Loesche leere Unterfelder in der Feldmitte und am Feldenede
         $line =~ s/\$\$.$//g;
         if ( $line =~ /(\$\$1\(DE-588\).+?)(\$\$|$)/ ) {         #Pruefe ob $$1 (DE-588) * vorkommt
            $line =~ s/\$\$2.+?\$\$/\$\$/g;                       #Loesche allfaellig vorhandenes Unterfeld $$2
